<template>
  <div class="mobile-user-privileges">
    <div class="user-privileges-container">
      <div class="user-privileges-card">
        ...
        <ul class="user-privileges-ul privileges-boosts-ul">
          <li v-for="(boost, index) of userBoosts" :key="index"
            ...
            @click.stop="!boost.number ? $router.push(boost.path) : null">
            ...
            <div class="v-flex d-flex f-align-center privileges-item-cnt">
              ...
              <span class="inline-block line-height-0 privileges-arrow-icon">
                <svg>
      ...
      <div class="user-privileges-card">
        ...
        <ul class="user-privileges-ul privileges-commission-ul">
          <li v-for="(commission, index) of userCommissions" :key="index"
            ...
            @click.stop="!commission.open ? $router.push(commission.path) : null">
            <span class="inline-block line-height-0 privileges-item-icon">
              <svg>
...
<script>
import { planBoostsMixins } from '@/mixins'
export default {
  name: 'mobile-user-privileges-component',
  mixins: [ planBoostsMixins ],
  computed: {
    userBoosts () {
      ...
      const infos = {
        referee: {
          ...
          path: `/owner/referee?refer=${this.$route.path}`
        },
        recruit: {
          ...
          path: `/taverns?refer=${this.$route.path}`
        },
        tavernkeep: {
    ...
    userCommissions () {
      ...
      const infos = {
        referrer: {
          ...
          path: `/owner/referral/invitation?refer=${this.$route.path}`
        },
        recruit: {
