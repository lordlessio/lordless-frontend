  ...
  <div class="d-flex user-building-box">
    <div class="d-flex v-flex col-flex container user-candy-container">
      ...
      <transition name="ld-hide-in-fade">
        <div
          v-if="!userTaverns.total && !loading"
        ...
        <div
          ...
          <ul>
            <li v-for="tavern of userTaverns.list" :key="tavern.id"
              ...
              <div class="v-flex d-flex col-flex f-justify-between owner-taverinfo">
                <p class="d-flex f-align-center owner-tavertitle">
                  <span v-if="userHome && userHome.homeInfo.tavern.id === tavern.id" class="inline-block owner-tavermark">HOME</span>
                  <span class="text-nowrap owner-tavername">{{ tavern.name.zh }}</span>
                </p>
                <div>
                  <p class="owner-taverlevel">Level {{ tavern.chain.level }}</p>
                  <p class="owner-taverleftap">{{ tavern.apLeft }} AP remaining</p>
                </div>
              </div>
...
import { activatedMixins, publicMixins, initLoadingMixins } from '@/mixins'
export default {
  name: 'mobile-owner-taverns',
  mixins: [ activatedMixins, publicMixins, initLoadingMixins ],
  data: () => {
    return {
  ...
  computed: {
    ...
    scrollOpt () {
      return {
        history: true,
        text: `${splitAddress(this.userAddress, { before: 4, end: 2 })} Taverns`
      }
    }
  },
  components: {
  ...
  methods: {
    ...
    async getUserTaverns ({ lord = this.userAddress, pn = this.userTaverns.pn, ps = this.userTaverns.ps } = {}) {
      if (!lord) return
      this.loading = true
      const params = {
        pn,
